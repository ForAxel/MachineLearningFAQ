<center><font size=7>机器学习八股文</font></center>
...
#### macro-$F_1$ vs micro-$F_1$

很多时候我们有多个二分类混淆矩阵（例如多次训练与测试 / 多个数据集 / 多分类任务中每两两类别的组合等），这是我们希望在 $n$ 个二分类混淆矩阵上综合考察模型性能。

- macro-$F_1$ 

    分类别计算各个混淆矩阵（不同类别）精确率和召回率，再计算平均值，分别得到 macro-$P$、macro-$R$和对应的macro-$F_1$。或者可以对不同类别的F1-score直接取平均
    $$
...
- micro-$F_1$ 
    ...
    micro F1不需要区分类别，直接使用总体样本的准召计算 F1 score
    
    先将各个混淆矩阵的对应元素统计，再基于这些值计算出micro-$P$、micro-$R$和对应的micro-$F_1$. 
    $$
    \text{micro-}P = \frac{\sum_{i-1}^n{TP_i}}{\sum_{i-1}^n{TP_i}+\sum_{i-1}^n{FP_i}}, \qquad
        \text{micro-}R = \frac{\sum_{i-1}^n{TP_i}}{\sum_{i-1}^n{TP_i}+\sum_{i-1}^n{FN_i}},
    $$
    $$
...
<img src="imgs/ROC_01.png" width="80%" />
...
<img src="imgs/ROC_02.png" width="80%" />

#### 关于ROC与AUC的补充

ROC曲线的生成方式

> 1. 对于分类器给出的每个样本，根据其得分排序。
> 2. 从最低得分开始，按顺序将每个样本作为正例，其他样本作为负例，计算此时的TPR和FPR。
> 3. 将计算得到的（FPR, TPR）坐标点绘制到ROC曲线上。
> 4. 重复步骤2和3，直到所有样本都被作为正例计算过一次为止。

> **如果模型A的ROC曲线被模型B的ROC曲线完全包含，并不能直接说明模型B优于模型A，因为ROC曲线的形状可以受到阈值的影响。**
>
> 虽然模型B的ROC曲线完全包含模型A的ROC曲线，但模型B可能会在某些阈值下表现得更差，而在其他阈值下表现得更好。因此，**在比较两个模型的分类性能时，需要综合考虑它们在不同阈值下的表现，而不仅仅是比较ROC曲线的形状和AUC值。**

> AUC的取值范围为0.5到1的原因是：考虑一个随机的二分类模型，其预测结果是随机的，即正例和负例的预测概率相等，这时TPR和FPR的值是相等的，ROC曲线的形状就是对角线（斜率为1），其AUC的值为0.5。而对于一个优秀的二分类模型，它的ROC曲线会离对角线越远越好，其AUC的值会越接近1。所以，AUC的取值范围为0.5到1，其中0.5表示分类器的分类性能等同于随机猜测，1表示分类器的分类性能完美。如果AUC的值小于0.5，则分类器的性能比随机猜测还差。
>
> **AUC值受数据分布和类别比例的影响**。在极度不平衡的数据集中，AUC值可能会高，但分类器的性能可能并不好。比如，对于一个包含99%负样本和1%正样本的数据集，如果分类器总是将所有样本预测为负样本，那么它的AUC值将会非常接近1，但其实分类器并没有实现真正的分类。此外，**AUC不能反映出分类器的准确性，而只是对分类器在不同阈值下的分类性能进行了平均化**。
...
### 相对熵与交叉熵 Ralative-Entropy / Cross-Entropy
...
**相对熵** (Ralative Entropy)，又称 **KL散度** (Kullback-Leibler Divergence)，是两个随机分布 $p$ 与 $q$ 之间的对数差值的期望
$$
...
- 全概率：若事件 B1, B2, ..., Bn 构成一个**完备事件组**，即他们两两不相容，且和为全集，则对于任意事件 A 有：
...
- 先验概率与后验概率
    ...
    > 先验概率是在考虑任何新数据或证据之前，我们已经拥有的有关事件的概率分布。它是**基于以往的经验和先前的知识，计算出来的概率**。举个例子，如果我们要预测明天是否会下雨，我们可以基于历史天气数据、气象预报和其他相关因素，计算出先验概率，即明天会下雨的概率。
    >
    > 后验概率是在**考虑到新数据或证据之后，我们更新先前的概率分布得到的概率**。举个例子，如果我们已经计算出明天下雨的先验概率，但今天天空变得阴沉，风也变得越来越强，这些新的证据可以帮助我们更新我们的先验概率，并计算出明天下雨的后验概率。后验概率通常是我们更感兴趣的概率，因为它是基于我们拥有的最新信息计算出来的。
...
- **该分类器之所以被称为“朴素”贝叶斯，是因为模型假设待分类项 $x$ 的所有特征都是独立的事件**。
- 常见的朴素贝叶斯分类器有：
...
- 基于概率的分类有可能会不准确，分类的性能不一定很高

### Generative Model vs Discriminative Model 生成模型 / 判别模型

#### 生成模型

生成模型通过**学习联合概率** $P(X,Y)$，即特征 $x$ 与类别 $y$ 同时出现的概率，再对每一个类别求后验概率，取概率最大的类别作为预测结果，即 $P(Y|X) = \frac{P(X,Y)}{P(X)}$。
...
生成式模型主要通过学习样本，形成多个Y分布，然后计算联合概率P(X,Y)，根据P(X,Y)的值预测新的样本属于哪个类。对于二分类问题，如果P(X,Y1)>P(X,Y2)，则新样本X判定为Y1

生成模型之所以叫生成模型，背后思想——什么样的标签对应生成什么样的特征，在根据x判断y的时候，实际上是在比较什么样的标签y更容易生成特征x，预测的结果就是更可能生成x特征的y标签

优缺点：

- 优点：过拟合几率小，采集的数据和真实整体数据相似时基本不用担心过拟合问题；
- 缺点：容易受到异常值的影响，异常点会影响分布函数；需要大量的数据来保证生成的分布函数与真实世界中的分布函数尽可能接近；模型计算量大

#### 判别模型

判别模型则是通过**学习条件概率**，学习输入变量和输出变量之间的映射关系，即直接预测特征 $x$ 下类别 $y$ 的概率。
...
- 常见生成模型
  - LDA、朴素贝叶斯、隐马尔可夫模型、高斯混合模型、变分自编码器VAE
- 常见判别模型
  - KNN、线性回归、logistic回归、SVM、决策树、神经网络
